...
export class InterviewsPairComponent implements OnInit {
  ...
  privremen: PairProgramming = new PairProgramming();
  flag:boolean = false;
  ...
  async test() {
    if (this.firstOpen) {
      ...
      let neupareni = this.service.getUnpairedInterviewPair(this.selectedType).subscribe(
      data => {
        if (data != null) {
          if (data.length % 2 == 0) {
            ...
            let nova = new PairProgramming();
            const num = 59 + Math.floor(Math.random() * 9);
              this.service.getQuestionPair(num).subscribe(
                  data => {
                    nova.pitanje=data.question;
                    nova.ucesnik1 = this.user.korime;
                    nova.status = "pending";
                    this.service.createInterviewPairRequest(nova).subscribe();
                  }
                );
            ...
            //this.service.pairInterviewPair(this.privremen).subscribe();
            //SACEMO DA SE MUCIMO SA TUDJIM KALENDAROM ----------------------------------------------------------------------------------

            this.service.getPotvrdjeneSimulacijePair(this.privremen.ucesnik1).subscribe(
              async data => {
                if (data != null) {
  ...
  ngOnInit(): void {
    ...
    this.service.getPotvrdjeneSimulacijePair(this.user.korime).subscribe(
      async data => {
        if (data != null) {
  ...
  confirmedSimulations: PairProgramming[] = [];
  previousSimulations: PairProgramming[] = [];

  confirmedSimulationsPaired: PairProgramming[] = [];
  previousSimulationsPaired: PairProgramming[] = [];
